...
class DarknetMonitor:
    ...
    def full_scan(self) -> Dict[str, Any]:
        ...
        # Keep alerts list synchronized
        self.alerts = []
        for t in results.get('threat_intel', []):
            self.alerts.append({
                'type': f"THREAT_{t.get('category', 'GENERAL').upper()}",
                'message': f"{t.get('title')} [{t.get('source')}]",
                'risk': t.get('category', 'medium').upper(),
                'time': t.get('detected', datetime.now().isoformat())
            })
        for b in results.get('bounty_leads', []):
            self.alerts.append({
                'type': 'BOUNTY_LEAD',
                'message': b.get('title', 'Bounty Opportunity'),
                'risk': 'MEDIUM',
    ...
    def search_darknet(self, query: str, max_results: int = 8) -> List[Dict[str, Any]]:
        """
        Deep multi-engine search across Tor darknet search engines (Ahmia & mirrors).
        Includes 8-second timeouts and multi-endpoint fallback.
        """
        import urllib.parse
        ...
        endpoints = [
            # 1. Ahmia clearnet via Tor proxy
            f"https://ahmia.fi/search/?q={encoded_q}",
            # 2. Ahmia Onion endpoint
            f"http://juhanurmih5wu7bv5imwt5rnodlaizm5zqujwcduvdhkr6ydq7i35zpad.onion/search/?q={encoded_q}",
            # 3. DuckDuckGo Onion
            f"https://duckduckgogg42xjoc72x3sjasowoarfbgcmvfimaftt6twagswzczad.onion/html/?q={encoded_q}"
        ]

        results = []
        print(f"🌑 [DarknetMonitor] Querying Tor darknet engines for '{query}'...")

        for ep in endpoints:
            try:
                resp = self._tor_get(ep, timeout=8)
                if resp and resp.status_code == 200:
                    ...
                    if "ahmia" in ep:
                        ...
                        for it in items[:max_results]:
                            title_tag = it.find('h4') or it.find('a')
                            title = title_tag.get_text().strip() if title_tag else "Darknet Result"
                    ...
                    # Parse DuckDuckGo Onion
                    elif "duckduckgo" in ep:
                        items = soup.find_all('div', class_='result__body')
                        for it in items[:max_results]:
                            title_tag = it.find('a', class_='result__snippet') or it.find('a')
                            title = title_tag.get_text().strip() if title_tag else "DDG Result"
                            snippet = it.get_text().strip()
                            if title:
                                results.append({
                                    "title": title,
                    ...
                    if results:
                        break  # Successfully pulled from active mirror
            except Exception as e:
                continue

        return results[:max_results]

    def scrape_deep_article(self, url: str) -> Dict[str, Any]:
        """
        Follow article link through Tor to extract full IOCs, CVEs, and details.
        Gracefully falls back if blocked by anti-bot.
        """
        from bs4 import BeautifulSoup
        print(f"🕵️ [DarknetMonitor] Fetching deep article context via Tor from {url[:50]}...")
        ...
        if not resp or resp.status_code != 200:
            return {
                "success": False,
                "url": url,
                "note": "Direct fetch blocked or timed out; using feed cache."
            }
        ...
        try:
            ...
            onions = list(set(re.findall(r'[a-z2-7]{16,56}\.onion', text, re.IGNORECASE)))

            return {
                "success": True,
                "url": url,
                "cves": cves,
                "linked_onions": onions,
    ...
    def cluster_threat_signals(self, results: Optional[Dict[str, Any]] = None) -> Dict[str, List[Dict[str, Any]]]:
        """
        Triage darknet threats into the operator's 3 strategic tiers:
        - Tier 1: Actionable 0-Days, RCE, Auth Bypass, High Bounties
        - Tier 2: Secondary enterprise framework patches
        - Tier 3: General background noise
        """
        res = results or self.last_scan_results or {}
        threats = res.get('threat_intel', [])
        bounties = res.get('bounty_leads', [])
        ...
        for t in threats:
            ...
            score = t.get('score', 0)
            if score >= 10 or any(k in title_lower for k in ['rce', 'zero-day', '0day', 'unauthenticated', '10m', 'exploit', 'ai exploit']):
                tier_1.append(t)
            elif score >= 3 or any(k in title_lower for k in ['patch', 'flaw', 'vulnerability', 'bypass']):
                tier_2.append(t)
            else:
                tier_3.append(t)
        ...
        for b in bounties:
            tier_1.append({
                ...
                'source': b.get('source', 'Bounty Feed'),
                'score': 15,
                'category': 'BOUNTY',
                'cves': b.get('cves', [])
            })
    ...
    def get_detailed_report(self, results: Optional[Dict[str, Any]] = None) -> str:
        """Generate a full, operator-grade itemized report of all findings with clustering."""
        ...
        lines = [
            ...
            "═" * 56,
            f"• Scan Time   : {res.get('scan_time', 'N/A')}",
            f"• Tor Exit IP : {res.get('tor_exit_ip', 'unknown')}",
            f"• Total Alerts: {res.get('total_alerts', 0)} ({res.get('critical_count', 0)} CRITICAL)",
            "═" * 56,
            "\n[ 1. 🔴 TIER-1 ACTIONABLE SIGNALS & BOUNTIES ]"
        ]

        if clustered["tier_1_actionable"]:
            for i, t in enumerate(clustered["tier_1_actionable"], 1):
                cves = f" (CVEs: {', '.join(t['cves'])})" if t.get('cves') else ""
                lines.append(f"  {i:02d}. [{t.get('category', 'THREAT').upper()}] {t.get('title')}{cves}")
        else:
            lines.append("  No Tier-1 critical signals active.")

        lines.append("\n[ 2. 🟡 TIER-2 SECONDARY INTEL & PATCHES ]")
        if clustered["tier_2_secondary"]:
            for i, t in enumerate(clustered["tier_2_secondary"][:6], 1):
    ...
    def get_last_scan_context(self) -> str:
        """Returns compact text of latest scan findings for LLM prompt context."""
        if not self.last_scan_results:
            return ""
        res = self.last_scan_results
        threat_titles = [f"• {t.get('title')} ({t.get('category')})" for t in res.get('threat_intel', [])[:5]]
        bounty_titles = [f"• {b.get('title')}" for b in res.get('bounty_leads', [])[:3]]
        
        ctx = [
            f"Latest Darknet Scan ({res.get('scan_time', 'recent')}):",
            f"Total Alerts: {res.get('total_alerts', 0)}, Critical: {res.get('critical_count', 0)}."
        ]
        if threat_titles:
