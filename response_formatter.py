#!/usr/bin/env python3
# response_formatter.py - Clean CLI output for the autonomous system
...
class ResponseFormatter:
    """
    Makes terminal output clean, readable, and operator-grade.
    No clutter. No noise. Just signal.
    """

    def __init__(self):
        self.terminal_width = self._get_terminal_width()
        self.ai_color    = '\033[96m'   # Cyan for AI responses
        self.user_color  = '\033[93m'   # Yellow for user
    ...
    def print_response(self, response: str):
        """Print AI response — clean, readable."""
        print(f"\n{self.ai_color}{self.bold}System:{self.reset}", end=" ")
        formatted = self._wrap_text(response, indent=6)
        print(f"{self.ai_color}{formatted}{self.reset}")

    # Alias for backward compatibility (if needed)
    def print_ciph(self, response: str):
        """Legacy alias for print_response()."""
    ...
    def print_banner(self, ai_enabled: bool, module_count: int, memory_entities: int):
        ...
        print(f"\n{self.ai_color}{'█' * width}{self.reset}")
        print(f"{self.ai_color}{'█':<{width}}{self.reset}")
        title = "AUTONOMOUS SYSTEM"
        padding = (width - len(title)) // 2
        print(f"{self.ai_color}{'█'}{' ' * padding}{self.bold}{title}{self.reset}{self.ai_color}{' ' * padding}{'█'}{self.reset}")
        print(f"{self.ai_color}{'█':<{width}}{self.reset}")
        print(f"{self.ai_color}{'█' * width}{self.reset}")
