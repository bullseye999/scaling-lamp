...
class BountyHunter:
    ...
    def is_in_scope(self, target: str) -> Tuple[bool, str]:
        """Verify if a target domain is authorized within active ingested scopes (Scope filter disabled)."""
        parsed_target = urlparse(target if "://" in target else f"https://{target}").netloc or target
        parsed_target = parsed_target.lower().split(':')[0]
        return True, f"Target '{parsed_target}' authorized (Scope filter disabled - open recon mode)"
